...
exports.signup = catchAsync(async (req, res, next) => {
  ...
  const emailToken = newUser.createRandomToken('');
  ...
  try {
    ...
    await new Email(newUser, confirmEmailURL).sendEmailConfirmation();
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  const resetToken = user.createRandomToken('reset');
  await user.save({ validateBeforeSave: false });
...
exports.verifyEmail = catchAsync(async (req, res, next) => {
  ...
  // 2) If token has not expired, and there is user, mark the email as verified
  if (!user) {
    return next(new AppError('Token is invalid or has expired', 400));
  }
  user.verified = true;
